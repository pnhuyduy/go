...
export const maxTouchPoints = [0, 10]

// Chromium's Sec-CH-UA-Platform-Version client hint, which uses a per-OS numbering scheme
// unrelated to the marketing OS version (e.g. Windows 11 reports "15.0.0"+). Linux has no
// meaningful mapping and reports an empty string.
export const platformVersion: Record<TPlatform, string> = {
  win: "15.0.0",
  mac: "10.15.7",
  linux: "",
}
...
// formula on a known real data point (146 -> 6998) keeps arbitrary major versions
// plausible without having to hand-maintain a full version lookup table.
const CHROME_BUILD_ANCHOR = { version: 146, build: 6998 }
