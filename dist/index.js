...
var extractProxyInfo = async (proxy, serviceChecker, timeout = 5e3) => {
  ...
  if (!host2 || !port2) throw new Error("Invalid proxy");
  let agent;
  let data = null;
  ...
  if (protocol === "http") agent = new import_https_proxy_agent.HttpsProxyAgent(proxyAgent, { timeout });
  else if (protocol === "socks") agent = new import_socks_proxy_agent.SocksProxyAgent(proxyAgent, { timeout });
...
var webGpuVendorFamily = (webglVendor) => {
...
var maxTouchPoints = [0, 10];
var platformVersion = {
  win: "15.0.0",
  mac: "10.15.7",
  linux: ""
};
var platformUAStrings = {
  win: "Windows NT 10.0; Win64; x64",
  mac: "Macintosh; Intel Mac OS X 10_15_7",
  linux: "X11; Linux x86_64"
};
var platformNavigatorPlatform = {
  win: "Win32",
  mac: "MacIntel",
  linux: "Linux x86_64"
};
var CHROME_BUILD_ANCHOR = { version: 146, build: 6998 };
var CHROME_BUILD_STEP_PER_VERSION = 62;
var generateChromeBuildNumber = (version2) => {
  const delta = version2 - CHROME_BUILD_ANCHOR.version;
  return Math.max(1e3, Math.round(CHROME_BUILD_ANCHOR.build + delta * CHROME_BUILD_STEP_PER_VERSION));
};
var generateUserAgent = (version2, platform = "win") => {
  const build = generateChromeBuildNumber(version2);
  const patch = randomInt(0, 150);
  return `Mozilla/5.0 (${platformUAStrings[platform]}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version2}.0.${build}.${patch} Safari/537.36`;
};
...
var getNewFingerprint = (payload, opts = defaultOptions) => {
  ...
  if (payload.proxy.protocol) {
    const { protocol, host: host2, port: port2, username: username2, password: password2 } = payload.proxy;
    ...
    newGologinConfig.proxy.server = `${host2}:${port2}`;
  ...
  if (payload.proxy.protocol) {
    ...
    prefs.proxy = {
      mode: "fixed_servers",
      server: `${protocol}://${username2}:${password2}@${host2}:${port2}`
    };
  }
  return prefs;
};
var webrtcIpHandlingPolicy = {
  real: "default",
  alerted: "default_public_interface_only",
  disabled: "disable_non_proxied_udp"
};
var spawnArgs = (options, payload, fingerprint = {}, args = []) => {
  const { webrtc, fonts, language } = (0, import_defu.default)(fingerprint, defaultOptions);
  let params = [];
...
var version = "146";
var executablePath = `C:/Users/Admin/.gologin/browser/orbita-browser-${version}/chrome.exe`;
var proxyStr = "103.179.189.46:14545:kp20:kp20";
...
var startProfile = async () => {
  ...
  const userDataDir = import_path2.default.join(profileStorage, randomInt(0, 999999).toString());
