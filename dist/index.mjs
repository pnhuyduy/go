...
var getNewFingerprint = (payload, opts = defaultOptions) => {
  ...
  newGologinConfig.mediaDevices.audioInputs = randomInt2(0, 3);
  newGologinConfig.mediaDevices.audioOutputs = randomInt2(0, 3);
  newGologinConfig.mediaDevices.videoInputs = randomInt2(0, 3);
...
var profileStorage = path2.join(path2.resolve(), "profiles");
var version = "146";
...
var startProfile = async () => {
  ...
  const userDataDir = path2.join(profileStorage, randomInt(0, 999999).toString());
  await fs2.ensureDir(path2.join(userDataDir));
