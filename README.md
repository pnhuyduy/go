...
const run = async () => {
  ...
  const args = spawnArgs(
    ...
    profile,
    { version: 134, platform: 'win' }, // cùng options đã dùng ở getNewFingerprint
  )
...
động tốt trên Orbita 134. Nên tự kiểm tra lại bằng trang test fingerprint thật nếu quan trọng.
...
- `generateUserAgent(version: number, platform: TPlatform): string` — sinh UA hợp lệ cho **bất kỳ** major
  version Chrome nào (không cần bảng tra cứu — build number được suy ra tuyến tính từ mốc `134` → `6998`).
...
|---|---|---|---|
| `version` | `number` | `134` | Major version Chrome, dùng để sinh UA + build number. |
